...
describe('report generator', () => {
  ...
  it('produces JSON', () => {
    const out = generateReport(manifest, findings as any, scoring, 'json');
    const parsed = JSON.parse(out);
    expect(parsed.findings.length).toBe(1);
  });

  it('produces SARIF', () => {
    const out = generateReport(manifest, findings as any, scoring, 'sarif');
    const parsed = JSON.parse(out);
    expect(parsed.runs[0].results.length).toBe(1);
  });

  it('produces text', () => {
    const out = generateReport(manifest, findings as any, scoring, 'text');
